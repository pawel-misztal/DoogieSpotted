...
import { SendToAllUsers } from "../wss.js";
...
export async function getDogMatches(req, res, next) {
    try {
        ...
        const matchesFiltered = matchesFound.map((matchModel) => {
            ...
            const isLower = match.lowerDogId === Number.parseInt(dogId);
            return {
                id: match.id,
...
function GetDogId(req) {
    if (!req.params.dogId) throw Error("no dogId");
...
function GetMatchId(req) {
    if (!req.params.matchId) throw Error("no matchId");
...
function IsLowerDog(dogId, match) {
    if (!match || match.dataValues) throw Error("no match data found");
